...
final class ContentFilterService {
    ...
    private let logger = Logger(subsystem: Bundle.main.bundleIdentifier ?? "SuperCapture", category: "ContentFilterService")
    ...
    func applySettings(to filter: SCContentFilter, settings: SettingsStore) async throws -> SCContentFilter {
        ...
        // If both wallpaper and dock are shown and SuperCapture is shown, no need to rebuild the filter
        if settings.showWallpaper && settings.showDock && settings.showSuperCapture {
        ...
        for window in availableWindows {
            ...
            // Exclude SuperCapture's own windows if showSuperCapture is false
            if !settings.showSuperCapture && bundleID == Bundle.main.bundleIdentifier {
                excludedWindows.append(window)
                logger.debug("Excluding SuperCapture window: \(windowTitle)")
                continue
            }
