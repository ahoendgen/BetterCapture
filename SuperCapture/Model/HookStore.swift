...
final class HookStore {
    ...
    private let logger = Logger(subsystem: Bundle.main.bundleIdentifier ?? "SuperCapture", category: "HookStore")

    // MARK: - File Path

    private static var configDirectory: URL {
        URL.applicationSupportDirectory.appending(path: "SuperCapture")
    }
